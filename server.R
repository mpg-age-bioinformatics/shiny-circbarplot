...
gitversion <- function(){ 
  git<-read.csv("/srv/shiny-server/.git/modules/circbarplot/refs/heads/master", header=FALSE)
  git<-git$V1
  git<-toString(git[1])
  git<-substr(git, 1, 7)
  return(git)
...
futile.logger::flog.threshold(futile.logger::ERROR, name = "circbarplotLogger")
...
shinyServer(function(input, output, session) {
  ...
  plot.data <- reactive({
    ...
    vars <- names(D)
    if (input$group == ""){
      updateSelectInput(session, "group","Select group column", 
                        choices = c("--select--","row.names", vars), 
                        selected = "--select--")
    }
    if (input$individual == ""){
      updateSelectInput(session, "individual","Select individual column", 
                        choices = c("--select--","row.names", vars), 
                        selected = "--select--")
    }
    if (input$values == ""){
      updateSelectInput(session, "values","Select values column", 
                        choices = c("--select--","row.names", vars), 
                        selected = "--select--")
    }
    req(input$group != "--select--", input$group != "", 
        input$individual != "--select--", input$individual != "",
        input$values != "--select--", input$values != "")
    ...
    D[, 'individual']<-D[,  input$individual]
    D[, 'values']<-D[,  input$values]
